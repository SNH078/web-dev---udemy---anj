...
app.get("/jokes/:id",(req,res)=>{
  ...
  // joke loops through all of the jokes  in jokes array that has the id same as the given id 
  res.json(foundJoke);
});
...
app.get("/filter", (req, res) => {
  ...
  const filteredActivities = jokes.filter((joke) => joke.jokeType === type);
   // joke loops through all of the jokes  in jokes array that has the type same as the given type 
//end result is an array that match the given type 
...
app.patch("/jokes/:id", (req, res) => {
  ...
  //if their is a request regarding text then update the test as req.body.text
  // or if their is no such request  existingJoke.jokeText.
  const replacementJoke = {
    id: id,
...
app.delete("/jokes/:id", (req, res) => {
  ...
  const searchIndex = jokes.findIndex((joke) => joke.id === id);
  if (searchIndex > -1) {
    //remove 1 joke starting with index =searchIndex
...
// query parameter in auth of postman // key:value
